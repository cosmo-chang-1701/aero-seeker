...
| **Three.js** | 3D 渲染引擎 | 模型構建、材質運算、氣流流線系統 |
...
- **氣流流線系統**: 使用 `BufferGeometry` 優化上萬條流線的渲染效能。
- **流場算法**: 流線位置根據向量 `(vx, vy, vz)` 實時運算，並受到飛行器體積的「排斥」作用，模擬真實氣流繞流效果。
- **馬赫波**: 計算公式 $ \mu = \arcsin(1/M) $，動態調整激波錐 (Shock Cone) 的角度與透明度。

### 2. 2D 降級傳承 (`Fallback2DViewer.jsx`)
- 利用 Canvas 2D `context.stroke()` 繪製氣流流線。
